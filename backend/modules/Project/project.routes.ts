...
projectRoutes.post("/create", [verifyToken], (req: Request, res: Response) => {
  ...
  const readableId = readableURL(req.body.title[0].quote);

  Project.findOne({ readableId }, (err, projectDB) => {
    if (err) res.json({ ok: false, err });

    if (projectDB) {
      return res.json({
        ok: false,
        desc: "A project with that title already exists.",
      });
    } else {
      ...
      project.readableId = readableURL(req.body.title[0].quote);
...
projectRoutes.patch("/update", [verifyToken], (req: any, res: Response) => {
  ...
  Project.findOne({ readableId }, (err, projectDB) => {
    if (err) {
      ...
      fileSystem.deleteImagesNotIncludedIn(
        "projects",
        req.body._id,
        currentImages
      );
